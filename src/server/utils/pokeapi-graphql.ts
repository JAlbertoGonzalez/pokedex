...
const PokemonSpeciesSchema = z.object({
  pokemonspecies: z.array(z.object({
    id: z.number(),
...
export async function fetchPokemonList(limit = 10000, offset = 0): Promise<PokemonListItem[]> {
  const query = gql`
...
export async function searchPokemonSpeciesBySpanishName(regex: string): Promise<{ id: number; name: string }[]> {
  const query = gql`
  ...
  return data.pokemonspecies.map(s => ({ id: s.id, name: s.pokemonspeciesnames[0]?.name ?? "" }));
}
